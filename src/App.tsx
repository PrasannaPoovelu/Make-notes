...
function App() {
  ...
  const noteWithTags = useMemo(() => {
    return note.map((n) => {
      return {
        ...n,
        tags: tag.filter((t) => n.tagsIds.includes(t.id)),
      };
    });
  }, [note, tag]);
  ...
  function onCreateNote({ tags, ...data }: NoteData) {
    setNote((prev) => {
      return [
        ...prev,
        { ...data, id: uuidV4(), tagsIds: tags.map((t) => t.id) },
      ];
    });
  }
  ...
  function onUpdateNote(id: string, { tags, ...data }: NoteData) {
    setNote((prev) => {
      return prev.map((n) => {
        if (n.id === id) {
          return { ...n, ...data, tagsIds: tags.map((t) => t.id) };
        } else {
          return n;
        }
      });
  ...
  return (
    <Container className=" my-4">
      <Routes>
        <Route
          path="/"
          element={<NoteList availableTags={tag} notes={noteWithTags} updateTag={updateTag} deleteTag={deleteTags} />}
        />
        <Route
          path="/create"
          element={
